...
function getFilesUnderSection(sesskey) {
	...
	const selectorOptions = [
		...
		document.querySelectorAll(
			"[class*='course-content'] li[class*='section']"
		),
	...
	if (contents.length === 0) {
		console.warn("No content sections found with any selector");
		return [];
	}
	...
	return contents
		.map((content) => {
			...
			if (!sectionEl) sectionEl = content.querySelector(".sectionname");
			if (!sectionEl)
			...
			if (activities.length === 0) {
				activities = Array.from(content.querySelectorAll("div.mod"));
			}
			...
			return activities
				.map((activity) => ({
					...
					archorTag:
						activity.getElementsByTagName("a")[0] ||
...
function getFilesFromModernLayout(sesskey) {
	...
	allLinks.forEach((link) => {
		try {
			...
			let nameEl =
				activityContainer.querySelector(".instancename") ||
			...
			let type = "";
			const instanceNameEl =
				activityContainer.querySelector(".instancename");
			if (instanceNameEl) {
				// The type is usually in a span after the link or as lastChild
				const typeEl =
					instanceNameEl.querySelector("span") ||
					instanceNameEl.lastChild;
			...
			let sectionContainer =
				activityContainer.closest("[class*='section']");
			if (sectionContainer) {
				let sectionNameEl =
					sectionContainer.querySelector("h3") ||
...
function getFiles() {
	try {
		...
		if (allFiles.length === 0) {
			console.log(
				"No files found with standard selectors, trying modern layout approach..."
			);
